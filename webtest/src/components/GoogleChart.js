...
//TraceSize":parseInt(i[7]),"TraceNodes
//(trace,temporal,nontemporal,pairfilter,nodefilter,compression,windowsize,tracesize,tracenodes)
...
//'Compression,WindowSize' methods
  ...
  generatePoint(Name,temporal,nontemporal,color){
    ...
    this.data.push(["",parseFloat(temporal),parseFloat(nontemporal),Name,size]); //substring(0,7)
...
Toggel(){
  if(this.selected != null) {
    ...
    if(this.togglearr[this.selected[0].row]==true) {
      ...
      let nonCollisionFlag=true;

      for(var i=0;i<this.data.length;i++){
        if(this.data[i][3]==this.Namearr[this.selected[0].row].substring(0,7)){
          this.data[this.selected[0].row+1][3]=this.Namearr[this.selected[0].row].substring(0,7)+this.runningIndex.toString();
          this.runningIndex=this.runningIndex+1;
          nonCollisionFlag=false;
          break;
        }
      }
...
handleAddFromTable(){
    for(var i=0;i<this.RowsSelection.length;i++)
      this.generatePoint(this.RowsSelection[i]["Trace"],this.RowsSelection[i]["Temporal"],this.RowsSelection[i]["Nontemporal"],"#0000FF");
}
